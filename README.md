...
High level code running in a different process could do this:
...
def json_rule(cfg):
    yield from t8.rule_ten8t_json_file(file_name='results.json', max_age_minutes=60)


checker = t8.Ten8tChecker(check_functions=[json_rule])
checker.run_all()
```
...
Support is provided using `rule_ten8t_json_files` for loading multiple files via `pathlib` style globbing
