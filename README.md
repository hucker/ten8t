...
def check_flakey_func():
    yield Ten8tResult(
        status=something.flakey_func(),
        msg="Attempting..."
    )
```
...
passing many parameters to functions embedded in code.
...
### `Ten8t` JSON Result Import

In larger systems it is possible that many processes are checking the health of a system. Aggregating
these files to provide full system visibility in one result set is very useful. One could imagine a
highly distributed system where ALL systems generated state results and saved the results to a single
folder, then a single tool could load all of those files for you to view full system state.

`JSON` import is directly supported.

Checker instances can directly generate JSON output like this:
...
def rule1(cfg):
    yield (t8.TR(status=True, msg='rule1 status'))


def rule2(cfg):
    yield (t8.TR(status=True, msg='rule2 status'))


def rule3(cfg):
    yield (t8.TR(status=True, msg='rule3 status'))


checker = t8.Ten8tChecker(check_functions=[rule1, rule2, rule3])
checker.run_all()
checker.to_json("results.json")

```

High level code running in a different process could do this, noting that json_rule3's function ends with
the word `_files` rather than `_file` meaning that it will use file globbing:
...
def json_rule1(cfg):
    yield from t8.rule_ten8t_json_file(file_name='server1/results.json', max_age_minutes=60)


def json_rule2(cfg):
    yield from t8.rule_ten8t_json_file(file_name='server2/results.json', max_age_minutes=60)


def json_rule3(cfg):
    yield from t8.rule_ten8t_json_files(file_name='tool_center/results_*.json', max_age_minutes=60)


checker = t8.Ten8tChecker(check_functions=[json_rule1, json_rule2])
checker.run_all()
```

This code will take the results file (`results.json`) generated by the first code and import it into
its own checker.


The functions that load JSON data files each have a `max_age_min` that allows the code to generate errors
if stale data is detected.

It is worth noting that there is nothing that says that `ten8t` code needs to generate the JSON files. Anything
could match the JSON format for the result data. Write results to a JSON file that just had the status and msg fields
would be enough to meet most use cases.

__This is very useful in large systems.__
