...
def test_nothing_rc_file(func1, func2, func3):
    ...
    rc = ten8t.Ten8tRC()
    ch = ten8t.Ten8tChecker(check_functions=[func1, func2, func3], auto_setup=True, rc=rc)
    assert ch.phases == ['p1', 'p2', 'p3']
    assert ch.tags == ['t1', 't2', 't3']
    assert ch.ruids == ['ruid_1', 'ruid_2', 'ruid_3']
    assert ch.levels == [1, 2, 3]
...
def test_check_with_rc(func1, func2, func3):
    rc = ten8t.Ten8tRC(rc_d={'tags': 't1'})
    ch = ten8t.Ten8tChecker(check_functions=[func1, func2, func3], auto_setup=True, rc=rc)
    assert ch.tags == ['t1']

    rc = ten8t.Ten8tRC(rc_d={'phases': ['p1']})
    ch = ten8t.Ten8tChecker(check_functions=[func1, func2, func3], auto_setup=True, rc=rc)
    assert ch.phases == ['p1']

    rc = ten8t.Ten8tRC(rc_d={'ruids': ['ruid_1']})
    ch = ten8t.Ten8tChecker(check_functions=[func1, func2, func3], auto_setup=True, rc=rc)
    assert ch.ruids == ['ruid_1']

    # TODO Fix this.  Levels should be ints not string 
    #  of ints.
    # rc = ten8t.Ten8tRC(display_name='rc',levels=[1])
    # ch = ten8t.Ten8tChecker(check_functions=[func1, func2, func3], auto_setup=True,rc=rc)
    # assert ch.levels == ['1']
