...
def test_result_pre_hook_result_only():
    ...
    def hook_test():
        ...
        yield ten8t.Ten8tResult(status=True, msg="It works")
    ...
    sp_func = ten8t.Ten8tFunction(hook_test, pre_sr_hooks=[result_hook1])
    for result in sp_func():
        assert result.status is False
        assert result.msg == "It works"

    sp_func = ten8t.Ten8tFunction(hook_test, pre_sr_hooks=[result_hook2])
    for result in sp_func():
...
def test_result_pre_hook_use_class():
    ...
    def result_doc_string_to_msg(self: ten8t.Ten8tFunction, sr):
        """ Hook to change the doc string to the msg"""
        sr.msg = self.doc
        return sr

    sp_func = ten8t.Ten8tFunction(hook_test, pre_sr_hooks=[result_doc_string_to_msg])
    for result in sp_func():
...
def test_verify_post_changes_pre():
    ...
    sp_func = ten8t.Ten8tFunction(hook_test, pre_sr_hooks=[result_pre], post_sr_hooks=[result_post])
    for result in sp_func():
