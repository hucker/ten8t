...
# Centralized list of executor types for easy modification
exec_params = [ProcessPoolExecutor]


# exec_params = [ThreadPoolExecutor]
# exec_params = [ThreadPoolExecutor, ProcessPoolExecutor]
...
@pytest.mark.parametrize("executor_type", exec_params)
def test_verify_no_threads(executor_type):
    with pytest.raises(t8.Ten8tException) as e_info:
        ch = t8.Ten8tChecker(check_functions=[], auto_setup=True)
        tcheck = t8.Ten8tExecutor(ch, executor_type=executor_type)
        _ = tcheck.run_all()
    assert str(e_info.value) == "You must provide at least one package, module or function to check."


@pytest.mark.parametrize("executor_type", exec_params)
def test_verify_single_thread(func1, executor_type):
...
@pytest.mark.parametrize("executor_type", exec_params)
def test_simple_threads(func1, func2, func3, executor_type):
...
def test_sorted_threads(funcs_permutation, func1, func2, func3, executor_type):
...
@pytest.mark.parametrize("executor_type", exec_params)
def test_timed_threads(one_sec_func, two_sec_func, executor_type):
...
@pytest.mark.parametrize("executor_type", exec_params)
def test_many_threads(executor_type):
    """
    ...
    functions = [make_t8_function(f"thread{i:02d}", 1) for i in range(10)]
    ch = t8.Ten8tChecker(check_functions=functions, auto_setup=True)
    tcheck = t8.Ten8tExecutor(ch, executor_type=executor_type)
    start_time = time()
    _ = tcheck.run_all(max_workers=10)
    end_time = time()
    execution_time = end_time - start_time
...
def test_thread_execution(max_workers, executor_type):
    """
    ...
    num_tests = 10  # Total number of tests to run
